...
std::chrono::time_point<std::chrono::system_clock> startBoot, endBoot;
startBoot = std::chrono::system_clock::now();
  ...
  endBoot = std::chrono::system_clock::now();
  std::chrono::duration<double> elapsedBoot = endBoot - startBoot;
  //time_t tt = std::chrono::system_clock::to_time_t(endBoot);
  //std::cout << "bootstrap processing time, in Tree.cpp " << elapsedBoot.count() << "s.\n" << std::flush;
...
// While not all nodes terminal, split next node
  ...
  //std::cout << "Before while split in treeGrow\n" << std::flush;
  ...
  //std::cout << "splitNode loop processing time, in Tree.cpp " << elapsedTree.count() << "s.\n" << std::flush;
...
void Tree::createPossibleSplitVarSubset(std::vector<size_t>& result) {
  //std::cout << "In createPossibleSplitVarSubset\n" << std::flush;
  ...
  //std::cout << "In createPossibleSplitVarSubset, before draw\n" << std::flush;
  bool emp = split_select_weights->empty();
  if (emp){
    //std::cout << "In createPossibleSplitVarSubset, in emp\n" << std::flush;
  }else{
    //std::cout << "In createPossibleSplitVarSubset, in emp else\n" << std::flush;
  }
  //std::cout << "in createPossibleSplitVarSubset, after emp\n" << std::flush;
  // Randomly add non-deterministic variables (according to weights if needed)
  if (split_select_weights->empty()) {
    //std::cout << "In createPossibleSplitVarSubset, before drawWithoutReplacementSkip\n" << std::flush;
    ...
    //std::chrono::time_point<std::chrono::system_clock> startdraw, enddraw;
    //startdraw = std::chrono::system_clock::now();
    //std::cout << "In createPossibleSplitVarSubset, before num_draws\n" << std::flush;
    size_t num_draws = mtry - result.size();
    //std::cout << "In createPossibleSplitVarSubset, after num_draws\n" << std::flush;
    drawWithoutReplacementWeighted(result, random_number_generator, *split_select_varIDs, num_draws,
        *split_select_weights);

  //enddraw = std::chrono::system_clock::now();
  //std::chrono::duration<double> elapseddraw = enddraw - startdraw;
  //time_t tt = std::chrono::system_clock::to_time_t(enddraw);
  //std::cout << "drawWithoutReplacementWeighted processing time, in Tree.cpp " << elapseddraw.count() << "s.\n" << std::flush;
  }
}

bool Tree::splitNode(size_t nodeID) {
  //std::cout << "In splitNode\n" << std::flush;
  // Select random subset of variables to possibly split at
  std::vector<size_t> possible_split_varIDs;
  createPossibleSplitVarSubset(possible_split_varIDs);

  std::chrono::time_point<std::chrono::system_clock> startSplit, endSplit;
  startSplit = std::chrono::system_clock::now();


  // Call subclass method, sets split_varIDs and split_values
  bool stop = splitNodeInternal(nodeID, possible_split_varIDs);

  endSplit = std::chrono::system_clock::now();
  std::chrono::duration<double> elapsedSplit = endSplit - startSplit;
  //time_t tt = std::chrono::system_clock::to_time_t(endSplit);
  //std::cout << "splitNodeInternal processing time, in Tree.cpp " << elapsedSplit.count() << "s.\n" << std::flush;
  ...
  if (data->isOrderedVariable(split_varID)) {
    ...
    //std::cout << "In tree SplitNode, sampleID[nodeID] size: " << sampleIDs[nodeID].size() << '\n' << std::flush;
  // No terminal node
  return false;
}
