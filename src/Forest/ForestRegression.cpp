...
void ForestRegression::initInternal(std::string status_variable_name) {
  std::cout << "In initInternal\n" << std::flush;
  std::cout << "MtryType: " << mtryType << '\n' << std::flush;
  // If mtry not set, use floored square root of number of independent variables
  if (mtry == 0 && mtryType == MTRY_SQRT) {
    unsigned long temp = sqrt((double) (num_variables - 1));
    mtry = std::max((unsigned long) 1, temp);
    std::cout << "In mtry_sqrt\n" << std::flush;
  }
  else if (mtry == 0 && mtryType == MTRY_0_2) {
    unsigned long temp = 0.2 * ((double) (num_variables - 1));
    mtry = std::max((unsigned long) 1, temp);
    std::cout << "In mtry_0_2\n" << std::flush;
  }
  std::cout << "In initInternal, after mtry\n" << std::flush;
...
void ForestRegression::initInternalData(std::string status_variable_name) {
  std::cout << "In initInternal\n" << std::flush;
  // If mtry not set, use floored square root of number of independent variables
  std::cout << "MtryType: " << mtryType << '\n' << std::flush;
  // If mtry not set, use floored square root of number of independent variables
...
void ForestRegression::growInternal() {
  std::cout << "Starting growInternal() in ForestRegression.cpp\n";
  trees.reserve(num_trees);
  for (size_t i = 0; i < num_trees; ++i) {
...
void ForestRegression::writePredictionFile(int rank) {

// Open prediction file for writing
  std::string directory = outputDirectory;
  std::string rankStr = std::to_string(rank);
  std::string filename = directory + "/" + output_prefix + ".prediction";
  std::ofstream outfile;
  outfile.open(filename, std::ios::out);
  if (!outfile.good()) {
...
void ForestRegression::sendPredictionsInternal(std::vector<double> &preds){
  std::vector<double> predVec;
  int count = 0;
  for (size_t i = 0; i < predictions.size(); ++i) {
    for (size_t j = 0; j < predictions[i].size(); ++j) {
      for (size_t k = 0; k < predictions[i][j].size(); ++k) {
        predVec[count] = predictions[i][j][k];
        count++;
      }
    }
  }
...
void ForestRegression::sendTreesInternal(std::vector<double> &classValues, std::vector<uint> &responseClassIDs){
