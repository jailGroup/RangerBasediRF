...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry, MtryType mtryType,
    ...
    uint num_random_splits, int useMPI, int rank, int size, std::string outputDirectory, std::ostream* verbose_time) {
        ...
        *verbose_out << "Finished loading input file\n";
        ...
        std::cout << "Before init, mtry: " << getMtry() << '\n' << std::flush;
        init(dependent_variable_name, memory_mode, data, mtry, mtryType, output_prefix, num_trees, seed, num_threads, importance_mode,
        ...
        std::cout << std::flush;
        std::cout << "After init, mtry: " << getMtry() << '\n' << std::flush;
        if (prediction_mode) {
      ...
      void Forest::initCppData(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, uint mtry, MtryType mtryType,
          ...
          uint num_random_splits, int useMPI, int rank, int size, Data* oldData, std::string outputDirectory) {
              ...
              initData(dependent_variable_name, memory_mode, oldData, mtry, mtryType, output_prefix, num_trees, seed, num_threads, importance_mode,
                  min_node_size, status_variable_name, prediction_mode, sample_with_replacement, unordered_variable_names,
                  memory_saving_splitting, splitrule, predict_all, sample_fraction, alpha, minprop, holdout, prediction_type,
                  num_random_splits, useMPI);
              std::cout << "In Forest.cpp, directly after call to init\n";
              ...
              if (!split_select_weights_file.empty()) {
                ...
                if (mtryType == MTRY_SQRT) {
                  unsigned long temp = sqrt((double) nonzero_Weights);
                  mtry = std::max((unsigned long) 1, temp);
                  std::cout << "After setting mtry to sqrt(p): " << mtry << "\n" << std::flush;
                  std::cout << "nonZero weight count: " << nonzero_Weights << '\n' << std::flush;
                }
                else if (mtry == 0 && mtryType == MTRY_0_2) {
                  unsigned long temp = 0.2 * ((double) (nonzero_Weights));
                  mtry = std::max((unsigned long) 1, temp);
                  std::cout << "After setting mtry to 0.2*p: " << mtry << "\n" << std::flush;
                  std::cout << "nonZero weight count: " << nonzero_Weights << '\n' << std::flush;
                }
                this->mtry = mtry;
                std::cout << "forest getMtry: " << getMtry() << '\n' << std::flush;
                std::cout << "forest mtry: " << mtry << '\n' << std::flush;
...
void Forest::initR(std::string dependent_variable_name, Data* input_data, uint mtry, uint num_trees,
  ...
  // Call other init function
  init(dependent_variable_name, MEM_DOUBLE, input_data, mtry, MTRY_SQRT, "", num_trees, seed, num_threads, importance_mode,
      min_node_size, status_variable_name, prediction_mode, sample_with_replacement, unordered_variable_names,
      memory_saving_splitting, splitrule, predict_all, sample_fraction, alpha, minprop, holdout, prediction_type,
      num_random_splits, useMPI);
...
void Forest::init(std::string dependent_variable_name, MemoryMode memory_mode, Data* input_data, uint mtry, MtryType mtryType,
...
void Forest::initData(std::string dependent_variable_name, MemoryMode memory_mode, Data* input_data, uint mtry, MtryType mtryType,
    std::string output_prefix, uint num_trees, uint seed, uint num_threads, ImportanceMode importance_mode,
    uint min_node_size, std::string status_variable_name, bool prediction_mode, bool sample_with_replacement,
    std::vector<std::string>& unordered_variable_names, bool memory_saving_splitting, SplitRule splitrule,
    bool predict_all, double sample_fraction, double alpha, double minprop, bool holdout,
...
void Forest::grow() {
  ...
  *verbose_out << "Growing trees internal .." << std::endl;
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    std::cout << "In forest grow, mtry: " << mtry << '\n' << std::flush;
