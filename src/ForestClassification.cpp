...
void ForestClassification::initInternalData(std::string status_variable_name) {
...
void ForestClassification::growInternal() {
  ...
  *verbose_out << "Grow Internal: pushed back trees.." << std::endl;
}
...
void ForestClassification::writePredictionFile(int rank) {

  // Open prediction file for writing
  std::string rankStr = std::to_string(rank);
  std::string filename = output_prefix + rankStr + ".prediction";
  std::ofstream outfile;
  outfile.open(filename, std::ios::out);
  if (!outfile.good()) {
...
void ForestClassification::sendPredictionsInternal(std::vector<double> &preds){
...
void ForestClassification::sendTreesInternal(std::vector<double> &classValues, std::vector<uint> &responseClassIDs){
  classValues = this->class_values;
  responseClassIDs = this->response_classIDs;
}
// #nocov end
