...
void drawWithoutReplacementSkip(std::vector<size_t>& result, std::mt19937_64& random_number_generator, size_t max,
    std::vector<size_t>& skip, size_t num_samples) {
      //std::cout << "In drawWithoutReplacementSkip\n" << std::flush;
  if (num_samples < max / 10) {
    //std::cout << "In drawWithoutReplacementSkip, before drawWithoutReplacementSimple\n" << std::flush;
    drawWithoutReplacementSimple(result, random_number_generator, max, skip, num_samples);
  } else {
    //std::cout << "In drawWithoutReplacementSkip, before drawWithoutReplacementFisherYates\n" << std::flush;
...
void drawWithoutReplacementWeighted(std::vector<size_t>& result, std::mt19937_64& random_number_generator,
    std::vector<size_t>& indices, size_t num_samples, std::vector<double>& weights) {
      //std::cout << "In drawWithoutReplacementWeighted\n" << std::flush;
  result.reserve(num_samples);
  ...
  //std::cout << "in drawWithoutReplacementWeighted, after weighted_dist\n" << std::flush;
  for (size_t i = 0; i < num_samples; ++i) {
    //std::discrete_distribution<> weighted_dist(temp_weights.begin(), temp_weights.end());
    size_t draw;
    do {
      draw = weighted_dist(random_number_generator);
    } while (temp[draw]);
    temp[draw] = true;
    //temp_weights[draw] = 0.0;
