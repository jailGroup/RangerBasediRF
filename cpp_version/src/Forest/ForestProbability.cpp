...
void ForestProbability::initInternalData(std::string status_variable_name) {
...
void ForestProbability::writePredictionFile(int rank) {
...
void ForestProbability::sendPredictionsInternal(std::vector<int> &preds){
  //Currently empty until implemented, Probability not covered for MPI runs yet
}
...
void ForestProbability::sendTreesInternal(std::vector<double> &classValues, std::vector<uint> &responseClassIDs){
