...
void ForestSurvival::initInternalData(std::string status_variable_name) {
...
void ForestSurvival::writePredictionFile(int rank) {
...
void ForestSurvival::sendPredictionsInternal(std::vector<int> &preds){
  //Currently empty until implemented, Survival not covered for MPI runs yet
}
...
void ForestSurvival::sendTreesInternal(std::vector<double> &classValues, std::vector<uint> &responseClassIDs){
