...
ArgumentHandler::ArgumentHandler(int argc, char **argv) :
    caseweights(""), depvarname(""), fraction(1), holdout(false), memmode(MEM_DOUBLE), savemem(false), predict(""), predictiontype(
        ...
        DEFAULT_NUM_THREADS), predall(false), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), file(""), yfile(""), impmeasure(
        ...
        TREE_CLASSIFICATION), seed(0), outputDirectory("") {
  this->argc = argc;
  this->argv = argv;
}
...
int ArgumentHandler::processArguments() {
  ...
  // short options
  char const *short_options = "A:C:D:F:G:HI:J:K:L:M:NP:Q:R:S:U:XZa:b:c:f:g:hil::m:o:pr:s:t:uvwy:z:";
  ...
  // long options: longname, no/optional/required argument?, flag(not used!), shortname
    const struct option long_options[] = {
      ...
      { "groupNumber",          required_argument,  0, 'G'},
      { "holdout",              no_argument,        0, 'H'},
      { "useMPI",				        required_argument,	0, 'I'},
      { "numIterations",        required_argument,  0, 'J'},
      { "printPathfile",        required_argument,  0, 'K'},
      { "outputDirectory",      required_argument,  0, 'L'},
  ...
  while (1) {
    ...
    case 'I':
      // std::cout << "In case I\n";
    	// useMPI = true;
      useMPI = atoi(optarg);
      std::cout << "Use MPI in ArgHandler: " << useMPI << '\n';
      if (useMPI == 1){
        std::cout << "Use MPI\n";
      }
      else if (useMPI == 0){
        std::cout << "Do not use MPI\n";
      }

      else{
        //throw std::runtime_error("Not a valid value for useMPI, must be either 0 or 1: 0 - Do not use MPI or 1 - Use MPI");
        std::cout << "Else MPI value: " << useMPI << '\n';
      }
    	break;
    ...
    case 'n':
      try {
        ...
        throw std::runtime_error(
            "Illegal argument for option 'mtryType'. Please give a positive integer. See '--help' for details.");
      }
      break;
...
void ArgumentHandler::displayHelp() {
  ...
  std::cout << "    " << "--yfile FILE                  Filename of single column y input data. (optional, --file treated as X only)" << std::endl;
  ...
  std::cout << "    " << "--ntree N                     Set number of trees PER COMPUTE NODE to N." << std::endl;
  ...
  std::cout << "    " << "--mtryType type               Set mtry type to:" << std::endl;
  std::cout << "    " << "                              TYPE = 1: sqrt(p) with p = number of independ variables" << std::endl;
  std::cout << "    " << "                              TYPE = 2: 0.2*p with p = number of independ variables" << std::endl;
  std::cout << "    " << "                              (Default: 1)" << std::endl;
  ...
  std::cout << "    " << "--useMPI VALUE                VALUE 0: Do not use MPI" << std::endl;
  std::cout << "    " << "                              VALUE 1: Use MPI Parallel computing, a forest per computation node." << std::endl;
  std::cout << "    " << "--outputDirectory DIRECTORY   Directory: Do not include '/' at end. Must list a directory." << std::endl;
  std::cout << "    " << "                              If directory does not exist, will be created. PARENT DIRECTORY MUST EXIST." << std::endl;
  std::cout << std::endl;
